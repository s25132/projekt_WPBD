...
after_schema = T.StructType([
    ...
    # Debezium Decimal jako "bytes" → w JSON to base64; mapujemy na BinaryType
    T.StructField("amount",       T.BinaryType(),  False),
    # io.debezium.time.MicroTimestamp → int64 (liczba mikrosekund)
...
flat = (parsed
    ...
    .select(
        ...
        debezium_decimal(F.col("payload.amount")).alias("amount"),
...
delta_q = (flat.writeStream
    ...
    .option("path", "s3a://datalake/tables/orders/")
    .start())
